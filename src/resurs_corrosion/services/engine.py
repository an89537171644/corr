...
from .capacity import evaluate_margin, find_limit_state_crossing
...
def run_scenario(
    ...
    def evaluate_at_age(age_years: float):
        cache_key = round(age_years, 10)
    ...
    remaining_life = find_limit_state_crossing(
        timeline,
...
def collect_fallback_flags(zone_observations: list, results: List[ScenarioResult]) -> List[str]:
    flags = []
    for observation in zone_observations:
        if observation.source != "baseline":
            continue
